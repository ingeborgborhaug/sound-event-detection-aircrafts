...
def safe_move(filename, classes, class_idx, retries=10):
        """
        Waiting max 0.1*retries seconds for the playback to be done, 
        before moving file to a class folder.
        """
        for i in range(retries):
                try:
                        shutil.move(os.getcwd()+'/'+filename, os.getcwd()+'/'+classes[class_idx]+'/'+filename)
                        return True
                except:
                        time_import.sleep(0.1)
...
def window_labeling(filename, classes, plot_feature,probability_default, probability_labeltype):
        ...
        #os.system(f'sox "{filename}" -d')
        print(f'filename: {filename}')
        playsound(filename)
        time_import.sleep(timesplit)
